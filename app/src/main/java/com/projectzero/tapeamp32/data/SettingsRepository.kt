...
object SettingsKeys {
    ...
    // BARU (fitur multi-folder): pengganti MUSIC_FOLDER_URI (satu string) --
    // sekarang bisa nyimpen BANYAK folder sekaligus (JSON array of string).
    // MUSIC_FOLDER_URI lama TETAP dipertahankan (tidak dihapus) supaya install
    // lama yang belum pernah buka versi ini tetap bisa dimigrasi otomatis --
    // lihat migrateSingleFolderIfNeeded() di PlayerViewModel.
    val MUSIC_FOLDER_URIS_JSON = stringPreferencesKey("music_folder_uris_json")
...
class SettingsRepository(private val context: Context) {
    ...
    val musicFolderUrisJson: Flow<String> = safeDataStore.map { it[SettingsKeys.MUSIC_FOLDER_URIS_JSON] ?: "" }
